  ...
  if (cur_config['num_pixels'] != strip_length) {
    ...
    probe_url_for(URL_TWILIGHT_SETUP + '?' + $.param({ [PARAM_NUM_PIXELS]: strip_length }),
      ...
      function (text) {
        ...
        setTimeout(config_update_sync, DEVMODE ? 100 : 0);
      });
    return;
  }
...
function recurrent_event_collect_data() {
  ...
  result['time'] = [Math.floor(time_start / 60).toFixed(0)];
  ...
  if (time_end_enable.is(":checked")) {
    ...
    result['time'].push(Math.floor(time_end / 60).toFixed(0));
  }
...
function print_event_desc(event) {
  ...
  if (event.time.length == 1) {
    const start_time = new Date(event.time[0] * 60 * 1000);
    result += `at ${time_formatter.format(start_time)}`;
  } else {
    const start_time = new Date(event.time[0] * 60 * 1000);
    const end_time = new Date(event.time[1] * 60 * 1000);
...
function prompt_recurrent_event_config(event, save_action) {
  ...
  if (event.type) {
    ...
    tdex_start.setTime(event.time[0] * 60);
    tdex_start.select('hr');
    if (event.time.length > 1) {
      tdex_end.setTime(event.time[1] * 60);
