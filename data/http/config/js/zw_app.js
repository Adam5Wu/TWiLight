...
function get_counter_color(color) {
  var result = new iro.Color();
  const color_h = color.hsv.h;
  result.setChannel('hsv', 'h', (color_h + 90 + (90 * color_h / 360)) % 360);
  const color_s = color.hsv.s;
  result.setChannel('hsv', 's', color_s < 30 ? 70 : (color_s < 60 ? 90 : color_s > 90 ? 60 : 80));
  const color_v = color.hsv.v;
  result.setChannel('hsv', 'v', color_v < 50 ? 80 : (color_v < 80 ? 90 : color_v < 90 ? 100 : 80));
  return result;
}
...
function print_transition_params(transition) {
  ...
  if ('color' in transition) {
    var bg_color = new iro.Color(transition['color']);
    var fg_color = get_counter_color(bg_color);
    text += `<br>Color: <span style="color:${fg_color.hexString};background-color:${bg_color.hexString};
    border-radius:1em">&nbsp;${transition['color']}&nbsp;</span>`;
  }
  ...
  probe_url_for(URL_TWILIGHT_SETUP + '?' + $.param({ "state": "exit-discard" }),
    function () {
      ...
      probe_url_for(URL_TWILIGHT_SETUP, function (payload) {
        ...
        block_prompt(`<p><center>TWiLight unavailable: ${text}</center>`);
      });
    }, function (text) {
      $("#config-form").removeClass("syncing");
      notify_prompt(`<p>Unable to exit setup mode:<br>${text}`);
  });
}
...
function transition_setup_uniform_color(row, cell, transition) {
  ...
  var dialog = $("#transition-params\\:uniform-color");
  var duration_slider = dialog.find("#uniform-color\\:duration-slider");
  var color_picker = dialog.find("#uniform-color\\:color-picker").data("color-picker");
...
$(function () {
  ...
  // Initialize transition params config dialogs
  ...
  $("dialog.appconfig").each(function (index, element) {
    var dialog = $(element);
