...
#define TWILIGHT_LIGHTSHOW_TASK_PRIORITY configMAX_PRIORITIES  // LS::kDefaultTaskPriority
...
esp_err_t _init_twilight(void) {
  ...
  ESP_LOGI(TAG, "Setting up LightShow...");
...
utils::ESPErrorStatus Setup_Exit(bool save_changes) {
  ...
  if (save_changes) {
    config_ = *state_.config_setup;
    {
      ...
      ESP_LOGI(TAG, "Storing TWiLight configuration...");
