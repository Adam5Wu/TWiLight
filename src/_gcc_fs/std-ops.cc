...
inline int symlink(const char *, const char *) { return -1; }
	 ...
	 error_code& ec)
{
  ...
  // 2682. filesystem::copy() won't create a symlink to a directory
	...
	options |= static_cast<copy_options>(4096);
      ...
      for (;iter;++iter)
	copy(iter->path(), to/iter->path().filename(), options, ec);
    }
  // _GLIBCXX_RESOLVE_LIB_DEFECTS
...
#ifdef __ESP_FS_RMDIR__
inline bool
esp_fs_remove(const fs::path& p, bool is_dir, std::error_code& ec) noexcept
{
  if ((is_dir? ::rmdir(p.c_str()) : ::remove(p.c_str())) == 0)
    {
      ec.clear();
      return true;
    }
...
fs::remove(const path& p, error_code& ec) noexcept
{
  ...
  return esp_fs_remove(p, s.type() == file_type::directory, ec);
}
...
#ifdef __ESP_FS_RMDIR__
  if (esp_fs_remove(p, s.type() == file_type::directory, ec))
#else
  if (fs::remove(p, ec))
#endif
    ++count;
